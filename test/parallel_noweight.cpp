...
int main(int argc, char* argv[]) {
  ...
  // cluster information
  typedef cluster::union_find::node_noweight fragment_t;
  ...
  for (auto& f : fragments) {
    if (f.is_root()) f.set_id(nc_s++);
  }
  std::vector<std::size_t> weight_s(nc_s, 0);
  for (auto& f : fragments) ++weight_s[cluster_id(fragments, f)];
  std::size_t wmax_s = 0;
  std::size_t w2_s = 0;
  for (auto& f : fragments) {
    if (f.is_root()) {
      std::size_t w = weight_s[cluster_id(fragments, f)];
  ...
  std::size_t nc_p = 0;
  for (auto& f : fragments) {
    if (f.is_root()) f.set_id(nc_p++);
  }
  std::vector<std::size_t> weight_p(nc_p, 0);
  for (auto& f : fragments) ++weight_p[cluster_id(fragments, f)];
  std::size_t wmax_p = 0;
  std::size_t w2_p = 0;
  for (auto& f : fragments) {
    if (f.is_root()) {
      std::size_t w = weight_p[cluster_id(fragments, f)];
